...
function saveComment(boardId) {
    ...
    function sendRequest(formData) {
        fetch(commentUrl, {
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error('네트워크 응답이 올바르지 않습니다. 오류 코드: ' + response.status);
                }
                return response.json();
            })
            .then(data => {
...
function updateComment(boardId, commentId) {
    ...
    const commentImageInput = document.getElementById('inputCommentImage-' + commentId);
