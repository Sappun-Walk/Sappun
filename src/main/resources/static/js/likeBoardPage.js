    ...
    function changeAllBoardPage(element) {
    var page = element.getAttribute('data-page');
    window.location.href = boardUrl + '/all' + '?page=' + page;
}

    function changelikeBoardPage(element) {
    var page = element.getAttribute('data-page');
    window.location.href = boardUrl + '/likes' + '?page=' + page;
}
    ...
    window.location.href = boardUrl + '/reports';
} else {
    ...
    if (data.code === 200) {
    alert('신고 취소가 성공적으로 이루어졌습니다.');
    window.location.href = commentUrl + '/reports';
} else {
    alert('신고 취소 실패: ' + data.message);
}
})
