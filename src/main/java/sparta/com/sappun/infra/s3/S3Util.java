...
public class S3Util {
    ...
    ////        byte[] encodedFileName = Base64.getEncoder().encode("한글파일명".getBytes());
    ////        // 주어진 파일 URL로부터 파일명을 추출
    ////        String fileName = getFileNameFromFileUrl(fileUrl, filePath);
    ////        // 파일명이 비어있거나 해당 파일이 존재하지 않으면 예외 발생
    ////        if (fileName.isBlank()
    ////                || !amazonS3Client.doesObjectExist(bucketName, filePath.getPath() +
    ////                fileName)) {
    ////            throw new GlobalException(NOT_FOUND_FILE);
    ////        }
    ////        // S3에서 파일 삭제
    ...
    //        }
    //        // S3에서 파일 삭제
    //        amazonS3Client.deleteObject(bucketName, filePath.getPath() + encodedFileName);
    //    }
    //
