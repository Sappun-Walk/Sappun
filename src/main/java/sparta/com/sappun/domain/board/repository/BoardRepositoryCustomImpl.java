...
public class BoardRepositoryCustomImpl implements BoardRepositoryCustom {
    ...
    @Override
    public Page<Board> findLikeBoardsByUserId(Long userId, Pageable pageable) {
        List<Long> boardIds =
                queryFactory
                        .select(qLikeBoard.board.id)
                        .from(qLikeBoard)
                        .where(qLikeBoard.user.id.eq(userId))
                        .fetch();
        ...
        Long total =
                queryFactory.select(qBoard.count()).from(qBoard).where(qBoard.id.in(boardIds)).fetchOne();
